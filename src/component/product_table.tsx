...
const ProductTable: React.FC<ProductTableProps> = ({ products, sortKey, sortOrder, onSort }) => {
  ...
  const renderSortIcon = (key: ProductSortKey) => {
    if (sortKey !== key) return <ChevronDown className="h-4 w-4 text-gray-400" />;
  ...
  const groupedProducts = useMemo<GroupedProduct[]>(() => {
    ...
    const groups = Array.from(buckets.entries()).map(([groupId, items]) => {
      ...
      return { groupId, primary: sortedItems[0], items: sortedItems } satisfies GroupedProduct;
    });
      ...
      {selectedGroup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-white p-6 rounded-lg w-96 shadow-lg">
            ...
            <p className="text-gray-800"><strong>Nomer Induk Barang:</strong> {selectedGroup.primary.nomer_induk_barang}</p>
            <p className="text-gray-800"><strong>Kategori:</strong> {selectedGroup.primary.kategori_bisnis}</p>
            <p className="text-gray-800"><strong>Lokasi Barang:</strong> {selectedGroup.primary.lokasi_barang || "-"}</p>
            <p className="text-gray-800"><strong>Stok Barang:</strong> {selectedGroup.primary.stok_barang || "-"}</p>
            <p className="text-gray-800"><strong>Harga Produk:</strong> {selectedGroup.primary.harga_produk}</p>
